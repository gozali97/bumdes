            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header bg-card-primer">
                        <h3 class="card-title">Merubah data banner</h3>
                    </div>
                    <div class="card-body">
                        <div class="col-md-6 offset-md-3 mt-5">
                            <form accept-charset="UTF-8" action="{{ route('admin.banner.update', $banner->id) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                            
                                <div class="form-group {{ $errors->has('title') ? 'has-error' : '' }}">
                                  <label class="text-success" for="title">Judul</label>
                                  <input type="text" name="title" class="form-control " id="tittle"
                                      value="{{ $banner->title  ?? old('title') }}" placeholder="Judul">
                                  @if ($errors->has('title'))
                              ...
                              <div class="form-group {{ $errors->has('description') ? 'has-error' : '' }}">
                                  ...
                                  <input type="text" name="description" class="form-control " id="description"
                                      value="{{  $banner->description ?? old('description') }}" placeholder="Keterangan">
                              ...
                              <div class="form-group">
                                  ...
                                  <input type="text" name="button" class="form-control " id="button"
                                     value="{{ $banner->button ?? old('description') }}" placeholder="Label Tombol">
                              </div>

                              <div class="form-group">
                                  <label class="text-success" for="url" required="required">Link</label>
                                  <input type="text" name="url" class="form-control " id="url"
                                  value="{{ $banner->url ?? old('url') }}" placeholder="Link">
                              </div>
